...
import {bibToApa} from "@/lib/utils";
import {useToast} from "@/components/ui/use-toast";
import {Toaster} from "@/components/ui/toaster";
import {LogoHeader} from "@/components/LogoHeader";

export default function BibtexToApa() {
    ...
    const handleSubmit = async (event: React.FormEvent<EventTarget>) => {
        ...
        if (isBibtexLengthValid) {
            ...
            const apaText = bibToApa(bibJSON, authors)
            setApa(apaText)
            setIsLoading(false)
        ...
        if (response.ok) {
            ...
            alert('Failed to convert BibTex to Apa');
        }
        setIsLoading(false);
    };
        ...
        <div>
            ...
            <div className="main-content grid grid-rows-4 justify-center items-center min-h-screen min-w-screen">
                ...
                <Card className="row-span-1">
                    <CardHeader className="text-center">
                        <CardTitle className="text-2xl">APA Citation</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="flex flex-col items-center justify-center space-y-2 bg-white rounded-xl">
                            <Textarea id="apa" value={apa} readOnly disabled/>
                            <Button disabled={apa === ""} onClick={() => {
